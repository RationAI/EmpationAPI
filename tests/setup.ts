...
export async function setupIntercept(polly, allUrls=false): Promise<void> {
    ...
    if (!interceptedUser) {
        throw "Invalid setupIntercept call: invalid intercepted user name: " + interceptedUser;
    }
    ...
    const interceptor = async (req, res) => {
        ...
        req.headers['Authorization'] = req.headers['Authorization'] || `Bearer ${context.token}`;
    };
