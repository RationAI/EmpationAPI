...
describe('rationai api', () => {
  ...
  it('create global item', async () => {
    ...
    const createdItem = (await rationAI.globalStorage.create(
      newItem,
    )) as GlobalItem;
    ...
    expect(queryResult.find((item) => item.id === createdItem.id)).toEqual(
      updatedItem,
    );
    ...
    expect(
      queryResult2.find((item) => item.id === createdItem.id),
    ).toBeUndefined();
  });
});
