...
export default class JobConfig {
    ...
    private async fetchJobConfigItem(appId: string): Promise<GlobalItem | undefined> {
      return (await this.context.query({references: [appId], data_types: [this.configDataType]})).find(Boolean)
    }
    ...
    async createJobConfig(appId: string, value: object): Promise<object | false> {
      ...
      return await this.context.createValue(value, `Job config of App`, undefined, appId, GlobalItemReferenceType.JOB, this.configDataType)
    }
