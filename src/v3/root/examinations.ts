...
export default class Examinations extends RootContext {
    ...
    async create(caseId: string, appId: string): Promise<WorkbenchServiceApiV3CustomModelsExaminationsExamination> {
    ...
    async get(examinationId: string): Promise<WorkbenchServiceApiV3CustomModelsExaminationsExamination> {
        const self = this.context;
