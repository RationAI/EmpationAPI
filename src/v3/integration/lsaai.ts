...
export default class LSAAIIntegration extends DefaultIntegration<LSAAIIntegrationOptions> {
  ...
  private async parseUserInstitutionsAndProjects() {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch user info! ${response.statusText}. ${data}`,
      );
    }
    ...
    const result = (this.userData = {
      projects: {
        ...
        string,
        { label: string; value: string; rights: (string | GroupName)[] }
      >,
      institutions: {
    ...
    try {
      ...
      if (entitlements) {
        ...
        console.warn(
          'User info data does not contain access information! Is OAUTH scope set correctly?',
        );
      }
    } catch (e) {
